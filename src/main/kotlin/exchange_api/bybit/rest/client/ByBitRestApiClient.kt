package exchange_api.bybit.rest.client

import com.fasterxml.jackson.databind.ObjectMapper
import exchange_api.bybit.Authorization
import exchange_api.bybit.rest.response.*
...
class ByBitRestApiClient(private val apikey: String, private val secret: String) {
    private val url = "api.bybit-global.com"
    ...
    private inline fun <reified T : Response> executeRequest(request: Request): T? {
        ...
        log.debug {
            try {
                ...
                buildString {
                    appendLine("=== ByBit REST API Request ===")
                    appendLine("URL: ${request.url}")
                    appendLine("Method: ${request.method}")
                    appendLine("Headers:")
                    request.headers.forEach { (name, value) ->
                        if (name.contains("SIGN", ignoreCase = true) || name.contains("KEY", ignoreCase = true)) {
                ...
                "Failed to read request for logging: ${e.message}"
            }
        }
        ...
        client.newCall(request).execute().body?.string()?.let { respBody ->
            ...
            log.debug {
                buildString {
                    appendLine("=== ByBit REST API Response ===")
                    appendLine("URL: ${request.url}")
            ...
            if ((resp.retCode == 0L || resp.retCode == null) && (resp.ret_code == 0L || resp.ret_code == null)) {
                log.info("ByBit API call successful: ${request.method} ${request.url.encodedPath} (${requestDuration}ms)")
                resp
            } else {
                log.error("ByBit API call failed: ${request.method} ${request.url.encodedPath} - ${resp.retMsg ?: resp.ret_msg}")
                throw ByBitRestException(resp.retMsg ?: resp.ret_msg!!, resp.retCode ?: resp.ret_code!!)
            }
        }
    } catch (t: Throwable) {
        val requestDuration = System.currentTimeMillis() - requestStartTime
        log.error("Error executing ByBit API request: ${request.method} ${request.url.encodedPath} (${requestDuration}ms)", t)
        throw t
    }
    }
