...
import exchange_api.bybit.websocket.ByBitApiWebSocketListener
import exchange_api.bybit.websocket.messages.requests.WebSocketMsg
...
class StreamByBitFuturesImpl(
    ...
    private var privateStreamUrl: String = "wss://stream.bybit-global.com/v5/private"

    override fun run() {
        val publicUrl = if (isFuture) "wss://stream.bybit-global.com/v5/public/linear"
        else "wss://stream.bybit-global.com/v5/public/spot"
        ...
        if (api != null && sec != null) {
            privateStream = ByBitApiWebSocketListener(
                api = api,
                sec = sec,
                url = "$privateStreamUrl?max_alive_time=1m",
                timeout = timeout,
                keepConnection = true,
