...
Текущая версия: v2.1.15. Формат: semver. Каждая запись содержит проблему, решение и затронутые файлы.
...
### v2.1.15 - Balance check on resume for pending orders (2026-02-26)

**Статус:** Реализовано

**Проблема:**
После спайка `redistributeGridAfterSpike()` создаёт множество ордеров с `orderId=null` в БД (pending, не отправлены на биржу). При `/resume` → `synchronizeOrders()` пытается отправить все pending ордера, но большинство (92/103) отклоняются с "Not enough balance". Бот стартует и повторно пытается синхронизировать, тратя API-вызовы и заполняя Telegram ошибками. Проверка баланса существовала только для `/start` (fresh start), но не для `/resume`.

**Решение:**
Добавлена проверка баланса в `Communicator.startBot()` для resume-пути (grid-боты):
1. Читает существующие ордера из БД и фильтрует pending (`orderId == null`)
2. Считает необходимый баланс: сумма amount для SELL, сумма (amount × price) для BUY
3. Учитывает leverage (делит на плечо)
4. Сравнивает с доступным балансом на бирже
5. При недостатке — возвращает ошибку, бот не запускается

**Файлы:** `Communicator.kt`

---

### v2.1.14 - Fix: resume safety check blocking pending orders after spike redistribute (2026-02-24)

**Статус:** Реализовано

**Проблема:**
После спайка, `redistributeGridAfterSpike()` обнуляет `orderId` у множества ордеров в БД (они ожидают отправки на биржу при следующем sync). При `/resume` → `synchronizeOrders()`, все ордера с `orderId=null` попадали в "missing" список наравне с потерянными ордерами. Если таких ордеров >30% (111 из 199 = 55% в инциденте 24.02) — safety check блокировал ВСЮ синхронизацию. Бот не мог ни отправить pending ордера, ни восстановить пропавшие.

**Решение:**
Разделение "missing" ордеров на две категории:
1. **Pending** (`orderId == null`) — запланированные, но не отправленные (из redistribute/range expansion). Всегда отправляются на биржу без safety check.
2. **Missing** (`orderId != null`) — были на бирже, но пропали. К ним применяется safety check (>30% = подозрительно).

Safety check теперь не блокирует pending ордера, даже если missing ордеров слишком много.
